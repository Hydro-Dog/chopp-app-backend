...
export class NotificationGateway extends BaseGateway {
  ...
  constructor(jwtMiddleware: WsJwtMiddleware) {
    super(jwtMiddleware);
  ...
  async sendNotificationToClients<T>(
    recipientUserIds: string[], // Массив userId, которым нужно отправить уведомления
    ...
    const recipientSessions = activeSessions.filter((session) => recipientUserIds.includes(session.userId));
